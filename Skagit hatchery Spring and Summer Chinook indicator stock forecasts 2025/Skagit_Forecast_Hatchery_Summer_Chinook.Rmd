---
title: Marblemount Summer Chinook Forecast
...
dat <- read_csv(file.path(datadir, "marblemount_summer_ck_run_size.csv"))
...
dat_age <- read_csv(file.path(datadir, "marblemount_summer_ck_run_age_comp.csv"))
...
pct_marked_sub_yearling <- dat_pct_marked%>%
  filter(stock == "summerIndicator" & fwAge == 0 & year %in% by.temp)%>%
...
forecast_sub_yearling%>%
  group_by(age)%>%
  kbl(caption = "Table 6. sub-yearling hatchery summer chinook forecast percent marked by age.",digits =2)%>%
  kable_classic(full_width = F, html_font = "Cambria")

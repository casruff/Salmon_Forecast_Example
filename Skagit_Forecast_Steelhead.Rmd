---
title: Skagit Steelhead Forecast
author: Skagit River Comanagers
output:
  html_document:
    code_folding: hide
    fig_caption: yes
...
#====================================
#get Skagit Steelhead data as example
#====================================
dat<-read_csv("https://raw.githubusercontent.com/casruff/Skagit-River-Steelhead-Forecast/master/data/skagit_sthd_esc.csv")%>%
  left_join(read_csv("https://raw.githubusercontent.com/casruff/Skagit-River-Steelhead-Forecast/master/data/skagit_sthd_catch.csv"))%>%
...
dat<-dat%>%
  ...
  mutate(
    ...
    lag2_PC2 = lag(`Principal Component scores (PC2)`,2),
    lag2_CFS = lag(scale(CFS),2),
    lag2_NPGO = lag(NPGO,2),
    lag1_NPGO = lag(NPGO,1)
    )

#select variables to include in analysis and subset data
vars<-c("lag2_CFS","lag2_NPGO")#,"lag2_PC1","lag2_PC2","lag1_NPGO") #what regression variables will we use?
dat<-data.frame(dat)%>%
...
ARIMA100_withcov<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_CFS","lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
...
steelhead_forecast <- write.csv(best_draws,"steelheadforecast.csv")

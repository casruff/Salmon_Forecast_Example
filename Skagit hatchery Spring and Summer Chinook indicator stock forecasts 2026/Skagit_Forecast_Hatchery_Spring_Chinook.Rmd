---
title: Marblemount Spring Chinook Forecast
...
dat <- read_csv(file.path(datadir, "marblemount_spring_ck_run_size.csv"))
...
dat_age <- read_csv(file.path(datadir, "marblemount_spring_ck_run_age_comp.csv"))

dat_pct_marked <- read_csv(file.path(datadir, "marblemount_ck_hatchery_releases.csv"))
...
NPGO<-read_table("https://www.o3d.org/npgo/data/NPGO.txt",skip=29,col_names=F,comment="#")%>%
 ...
 #NPGO <- NPGO[which(NPGO$Year %in% Yrlist$Year),]
...
vars<-c("lag3_NPGO") #what regression variables will we use?
dat<-data.frame(dat)%>%
...
ARIMA100_withcov<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag3_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
...
## remove the stack weighted ensemble
ensemble_results$ensembles <-  ensemble_results$ensembles%>%
  filter(!Model == "Stack_weighted")
#=================================
...
## compute last 5 year average age comp
age <- apply(dat_age[(dim(dat_age)[1]-5):dim(dat_age)[1],-1],2,FUN = mean)

forecast <- as.matrix(age) %*% as.matrix(mu[,2])
forecast[2] <- forecast[2]+forecast[6]
forecast[3] <- forecast[3]+forecast[7]
forecast[6] <- 0
forecast[7] <- 0 
#pct_marked_by_age
#sub-yearling
forecast_sub_yearling <- forecast[1:4,]

forecast_sub_yearling <- cbind(age = c(2,3,4,5),trs = forecast_sub_yearling)

by.temp <- max(Yrlist,na.rm=T) - c(2,3,4,5)

pct_marked_sub_yearling <- dat_pct_marked%>%
  filter(stock == "springHatchery" & fwAge == 0 & year %in% by.temp)%>%
  dplyr::arrange(desc(year))%>%
  dplyr::select(percentMarked)
  
forecast_sub_yearling <- cbind(forecast_sub_yearling,pct_marked_sub_yearling)

forecast_sub_yearling%>%
  group_by(age)%>%
  kbl(caption = "Table 6. sub-yearling hatchery spring chinook forecast percent marked by age.",digits =2)%>%
  kable_classic(full_width = F, html_font = "Cambria")

##
## only the age 4 fish would be projected to return in 2025 since there weren't any
## releases of yearling Chinook in BY 2020, 2022, and 2023. 
forecast_yearling <- forecast[8,]

forecast_yearling <- cbind(age = 5,trs = forecast_yearling)

by.temp <- max(Yrlist,na.rm=T) - 5

pct_marked_yearling <- dat_pct_marked%>%
  filter(stock == "springHatchery" & fwAge == 1 & year %in% by.temp)%>%
  dplyr::arrange(desc(year))%>%
  dplyr::select(percentMarked)
  
forecast_yearling <- cbind(forecast_yearling,pct_marked_yearling)

forecast_yearling%>%
  group_by(age)%>%
  kbl(caption = "Table 7. yearling hatchery spring chinook forecast percent marked by age.",digits =2)%>%
  kable_classic(full_width = F, html_font = "Cambria")
````

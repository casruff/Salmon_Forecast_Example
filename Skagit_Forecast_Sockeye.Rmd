...
author: Casey Ruff, modified original code from Thomas Buehrens 
output:
  html_document:
    code_folding: hide
    fig_caption: yes
...
dat <- read_csv(file.path(datadir, "baker_sockeye_SAR.csv"))
...
NPGO<-read_table("https://www.o3d.org/npgo/data/NPGO.txt",skip=29,col_names=F,comment="#")%>%
  ...
  mutate(Year=Year+1, lag1_NPGO = NPGO,lag2_NPGO = lag(NPGO,1))%>%
...
dat <- dat%>%
  ...
  left_join(Smolts)%>%
  left_join(NPGO)%>%
  dplyr::select(Year=Year,runsize_obs,lag1_log_SAR2,lag2_NPGO,lag2_log_smolts)
...
#     lag2_smolts = log(lag(smolts,2)),
#     #lag3_smolts = lag(smolts,3)
#     )
...
#vars<-c("lag1_log_SAR2","lag2_NPGO")

vars<-c("lag1_log_SAR2","lag2_log_smolts","lag2_NPGO") #what regression variables will we use?
dat<-data.frame(dat)%>%
...
#sockeye_cov <- data.frame(dat[,c(1,5:6)])
# pcdat<-sockeye_cov%>%
#   ungroup()%>%
#   column_to_rownames(var="Year")
# mod<-prcomp(pcdat)
# dat <- data.frame(dat,PC1_smolts = mod$x[,1],PC2_smolts = mod$x[,2])
...
FY <- 1 #forecast Years (Years forward to forecast)
...
# 
# ARIMA100_withcov_NPGO_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
#             Year=dat$Year,
#             xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag2_NPGO")%>%as.matrix(),
#             h=FY,
#             min=TY,
#             type="ARIMA",
#             order=c(1,0,0)
...
ARIMA100_withcov_NPGO<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
...
ARIMA100_withcov_smolts<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_log_smolts")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
...
ARIMA100_withcov_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2")%>%as.matrix(),
            h=FY,
            min=TY,
            type="ARIMA",
            order=c(1,0,0)
...
ARIMA111_withcov_NPGO_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA111_withcov_NPGO<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA111_withcov_smolts<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_log_smolts")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA111_withcov_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA010_withcov_NPGO_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA010_withcov_NPGO<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA010_withcov_smolts<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_log_smolts")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA010_withcov_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2")%>%as.matrix(),
            h=FY,
            min=TY,
...
#             order=c(1,1,1)
# )
# ARIMA101_withcov_NPGO_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
#             Year=dat$Year,
#             xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag2_NPGO")%>%as.matrix(),
#             h=FY,
#             min=TY,
...
ARIMA101_withcov_NPGO<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA101_withcov_smolts<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_log_smolts")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA101_withcov_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2")%>%as.matrix(),
            h=FY,
            min=TY,
...
# ARIMA001_withcov_NPGO_smolts_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
#             Year=dat$Year,
#             xreg=dat%>%dplyr::select("lag1_log_SAR2","lag2_log_smolts","lag2_NPGO")%>%as.matrix(),
#             h=FY,
#             min=TY,
...
ARIMA001_withcov_NPGO<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_NPGO")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA001_withcov_smolts<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag2_log_smolts")%>%as.matrix(),
            h=FY,
            min=TY,
...
ARIMA001_withcov_SAR1<-tsCV2(y=log(dat$runsize_obs[-dim(dat)[1]]),
            Year=dat$Year,
            xreg=dat%>%dplyr::select("lag1_log_SAR2")%>%as.matrix(),
            h=FY,
            min=TY,
...
forecasts<-summarize_forecasts(
   c("ARIMA100",
    "ARIMA100_withcov_NPGO_smolts_SAR1",
    "ARIMA100_withcov_NPGO",
    "ARIMA100_withcov_smolts",
    "ARIMA100_withcov_SAR1",
    "ARIMA111_withcov_NPGO_smolts_SAR1",
    "ARIMA111_withcov_NPGO",
    "ARIMA111_withcov_smolts",
    "ARIMA111_withcov_SAR1",
    "ARIMA010_withcov_NPGO_smolts_SAR1",
    "ARIMA010_withcov_NPGO",
    "ARIMA010_withcov_smolts",
    "ARIMA010_withcov_SAR1",
    "ARIMA010",
    #,"ARIMA111",
    "ARIMA101_withcov_NPGO_smolts_SAR1",
    "ARIMA101_withcov_NPGO",
    "ARIMA101_withcov_smolts",
    "ARIMA101_withcov_SAR1",
    "ARIMA101",
    "ARIMA001_withcov_NPGO_smolts_SAR1",
    "ARIMA001_withcov_NPGO",
    "ARIMA001_withcov_smolts",
    "ARIMA001_withcov_SAR1",
    "ARIMA001",
    "gam"
    # LFO10
    #,"gp_horseshoe"
...
forecasts[which(forecasts$Model %in% forecast_skill$Model[1:10]),]

#===========================
#Plot of final Year forecast
#===========================
ggplot(data=forecasts[which(forecasts$Model %in% forecast_skill$Model[1:11]),],aes(x=Year,y=Estimate,color=Model))+
...
write.csv(best_model,"sockeye_preseason_fcst_performance.csv")
...
sockeye_forecast <- write.csv(best_draws,"sockeyeforecast.csv")
